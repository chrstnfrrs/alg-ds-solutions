const sumStringsA = (a,b) => {
  ...
  const short = a.length > b.length ? b.split('').reverse() :  a.split('').reverse()
  let remainder = 0
  
  const sum = long.reduce((acc, num, index) => {
    const cur = Number(num) + (short.length > index ? Number(short[index]) : 0) + remainder
    const value = cur % 10
  ...
  const finalSum = sumReversed.charAt(0) == '0' ? sumReversed.substring(1) : sumReversed

const getString = (number, value, index) => value.padEnd(String(number).length - index, '0')

const expandedForm = (number) => String(number).split('').reduce((accumulator, value, index) => {
  if (value !== '0') {
    if (index !== 0) {
      accumulator += ' + '
    } 
